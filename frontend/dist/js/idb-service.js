...
class IDBService {
    ...
    static getAllIDBData() {
        return this.getDBPromise().then(db => {
            if (!db) return;
            db.transaction('restaurants').objectStore('restaurants').getAll();
        });
    }
    ...
    static getAllOfflineReviewsIDB() {
        return this.getDBReviewsPromise().then(db => {
            if (!db) return;
            return db.transaction('offline-reviews').objectStore('offline-reviews').getAll();
        });
    }

    static handleOfflineReviews() {
        console.log('handler offline review started...');
        return this.getAllOfflineReviewsIDB().then(reviews => {
            console.log(reviews);
            if (!reviews) return;
            ...
            reviews.forEach(review => {
                ...
                this.insertUserReviewToDB(body).then(() => {
